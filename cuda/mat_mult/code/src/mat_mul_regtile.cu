...
__global__ void k_mat_mul_regtile(float* dev_a, float* dev_b, float* dev_c, int a_num_rows, int b_num_cols, int dim_shared) {
    ...
    for (int t = 0; t < num_tile_steps; t++)
    {
        ...
        for (int i = 0; i < TILE_SIZE; i++)
        {
            ...
            for (int j = 0; j < REG_TILE_SIZE; j++) 
            {
                ...
                b_reg[j] = b_shared[i][threadIdx.x * REG_TILE_SIZE + j];
            }
...
void mat_mul_regtile(std::vector<float>& a, std::vector<float>& b, std::vector<float>& c, int a_num_rows, int b_num_cols) {
