...
def load_gpt2(path):
    ...
    return wte, wpe, n_embd, n_head, ln1_w, ln1_b, attn_w, attn_b, proj_w, proj_b
...
def attention(ln_out, attn_w, attn_b, proj_w, proj_b, n_head):
    ...
    Q = QKV[:, :n_embd         ].reshape(n_tokens, n_head, head_dim).transpose(1, 0, 2)   # [n_head, n_tokens, head_dim]
...
wte, wpe, n_embd, n_head, ln1_w, ln1_b, attn_w, attn_b, proj_w, proj_b = load_gpt2("../gpt2.bin")
...
print(f"layer_norm output: shape={ln_out.shape}, dtype={ln_out.dtype}")
print(ln_out)
print(f"attention output:  shape={attn_out.shape}, dtype={attn_out.dtype}")
print(attn_out)
